...
- Every request needs the identity header or it gets a 401, so a bare
  `curl` against `rake dev` is refused until you pass `Remote-User`
  (`PRO_TACTS_IDENTITY_HEADER` renames it, and the refusal says which
  name it read). The security of that rests on the app being reachable
  only through a proxy that writes the header itself — never bind it to
