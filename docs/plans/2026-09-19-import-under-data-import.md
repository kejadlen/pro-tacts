...
now names the state: `rake import:macos:clear` clears this Mac of a
plan's contacts, and a contact's final status is `cleared` — on the
host, in its groups, off this Mac. `Import::Remove` is `Import::Clear`.

Statuses already recorded as `removed` read as `cleared` (`Plan`
carries the old spelling beside the new), so a plan built before the
rename still finishes and files away; nothing is rewritten on disk.

## A finished plan files away

A plan whose every contact is cleared is done (`Plan#done?`). The
clear task files one away into `done/` as its last step, and the
