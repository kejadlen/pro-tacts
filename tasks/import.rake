...
namespace :import do
  ...
  task :status do
    ...
    # ✅ only once every contact is off the Mac — removed, the last
    # status — and ⏳ until then, so it can never disagree with the
    # next lines below. The plan names are the directories',
    # copy-pasteable into PLAN=.
    plans.each do |plan|
      done = plan.contacts.size - plan.with_status(nil).size
      marker = ImportTasks::TO_LAND.call(plan) || ImportTasks::TO_LEAVE.call(plan) ? "⏳" : "✅"
