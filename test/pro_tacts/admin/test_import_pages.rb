...
class AdminImportPagesTest < Minitest::Test
  ...
  # update them instead, at the top of its editor: new until the
  # toggle says otherwise, so nothing in the book changes by default.
  def test_a_card_like_a_contact_in_the_book_offers_to_update_it
    with_contacts({"jane" => STORED_JANE}) do |_store|
      ...
      assert_includes last_response.body,
                      %(<a href="/import/#{id}/0" role="tab" aria-selected="true">new contact</a>)
      assert_includes last_response.body,
